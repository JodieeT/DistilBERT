"""
Fine-tune distilbert-base-uncased on IMDb (optional extension to the SST-2 pipeline).

Reads the dataset via data/IMDB.py.

Writes:
    results/distilbert_imdb_model/        fine-tuned model + tokenizer
    results/distilbert_imdb_results.txt   summary parsed by make_figures.py
    results/distilbert_imdb_config.json   metrics in the format expected by make_figures.py
...
MAX_LENGTH = 256
OUTPUT_DIR = "results/distilbert_imdb_model"
SUMMARY_PATH = "results/distilbert_imdb_results.txt"
CONFIG_PATH = "results/distilbert_imdb_config.json"
...
def main():
    ...
    sample_dataset = dataset["test"].select(
